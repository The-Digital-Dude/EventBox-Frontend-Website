...
const CategoryCard: React.FC<CategoryCardProps> = ({ category }) => {
  ...
  return (
    <Link href={`/category/${slug}`} className="group interactive-tilt-card relative bg-white p-6 rounded-[10px] border border-gray-100 transition-all-custom flex flex-col items-center text-center hover:shadow-2xl hover:shadow-primary/10 hover:-translate-y-2 hover:border-primary/20">
      <div className={`interactive-tilt-media size-16 md:size-20 rounded-[10px] ${category.color} text-white flex items-center justify-center mb-5 rotate-3 group-hover:rotate-0 transition-transform shadow-lg shadow-black/5`}>
